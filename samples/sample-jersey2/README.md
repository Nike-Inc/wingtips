# Wingtips Sample Application - jersey2

Wingtips is a distributed tracing solution for Java 7 and greater based on the 
[Google Dapper paper](http://static.googleusercontent.com/media/research.google.com/en/us/pubs/archive/36356.pdf).

This submodule contains a sample application based on Jersey 2 that integrates Wingtips' `RequestTracingFilter` to
...
* Launch the sample by running the `./runSample.sh` script. It will bind to port 8080 by default. 
    * You can override the default port by passing in a system property to the run script, 
    e.g. to bind to port 8181: `./runSample.sh -Djersey2Sample.server.port=8181`
...
* `GET /sample/async` - A Jersey 2 async endpoint that waits for 100 milliseconds on another thread before 
...
* `GET /sample/async-error` - A Jersey 2 async endpoint that sets the timeout for the request to 100 milliseconds 
