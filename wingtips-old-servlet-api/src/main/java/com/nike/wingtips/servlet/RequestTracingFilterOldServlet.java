 ...
 *
 * <p>This specific class is targeted at Servlet 2.x environments, where async requests are not supported. For
 * Servlet 3+ environments you should use {@code RequestTracingFilter} from the {@code wingtips-servlet-api} module.
 *
 ...
 *
 * <p>ALSO NOTE: Subclasses that support async servlet requests (like {@code RequestTracingFilter} from the
 * {@code wingtips-servlet-api} module) should override {@link #isAsyncRequest(HttpServletRequest)} and {@link
 * #setupTracingCompletionWhenAsyncRequestCompletes(HttpServletRequest, TracingState)}.
 *
 * @author Nic Munroe
 */
@SuppressWarnings("WeakerAccess")
public class RequestTracingFilterOldServlet implements Filter {
    ...
    /**
     * This impl will always return false since Servlet 2.x does not support async requests, but for
     * subclasses that do support async requests this method should be overridden to return the value of {@code
     * request.isAsyncStarted()}.
     *
     * <p>See {@code RequestTracingFilter} from the {@code wingtips-servlet-api} module for an impl that supports async
     * servlet requests.
     *
    ...
    /**
     * This impl does nothing by default (it will never be called since {@link #isAsyncRequest(HttpServletRequest)}
     * always returns false), but for subclasses that do support async requests this method should be overridden to
     * provide a listener that will complete the given {@link TracingState} when the given async request finishes.
